...
Each case study has its own `config.yaml`. See `examples/auv/config.yaml` for the full format. All paths in `config.yaml` are resolved relative to the config file's directory.
...
model:
  ...
  api_type: "openai"         # Provider: "openai" or "anthropic"
  api_base_url: "https://api.openai.com/v1"   # API endpoint base URL
```
...
- **Output**: `result_term_extraction.json` - a JSON file containing the extracted concept-instance pairs with traceability back to requirement GIDs
...
- **Input**: `requirement_architecture.json` + `result_term_extraction.json`
...
- **Output**:
  - `result_DSL.emf` - the generated Emfatic metamodel
  - `result_dsl_extraction.json` - traceability from requirements to metamodel elements
...
- **Input**: `requirement_architecture.json` + `result_term_extraction.json` + `result_DSL.emf`
...
- **Output**:
  - `result_eol_program.eol` - the generated EOL program
  - `result_model_creation.json` - traceability from requirements to model elements
...
- **Output**:
  - `STM.txt` - the generated state machine in RoboChart DSL
  - `result_statemachine_extraction.json` - traceability from requirements to state machine elements
...
This runs all four stages in order using the default AUV example: `term` -> `dsl` -> `model` -> `statemachine`.
...
2. **Write your requirements** in the JSON format shown above and place them in `assets/`.

3. **Write a system description** in a plain text file (`assets/system_description.txt`). This gives the LLM agents context about the domain.

4. **Copy and adapt** chain-of-thought and few-shot example files from the AUV example (`examples/auv/assets/`).

5. **Create a `config.yaml`** pointing to your assets. Use `examples/auv/config.yaml` as a template.

6. **Run the pipeline**: `python run.py --config examples/my_project/config.yaml`

Existing case studies in the AUV example can serve as references:
